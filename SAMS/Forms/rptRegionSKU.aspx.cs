...
/// </summary>
public partial class Forms_rptRegionSKU : System.Web.UI.Page
{
    /// <summary>
    /// Page_Load Function
    /// </summary>
    ...
    private void ShowReport(int ReportType)
    {
        ...
        DataSet ds = RptSaleCtl.GetRegionSKUWiseSales(int.Parse(this.ddDistributorType.SelectedItem.Value), int.Parse(DrpPrincipal.SelectedValue.ToString()), CategoryID, Convert.ToInt32(ddlSKU.SelectedValue), int.Parse(drpDistributor.SelectedValue.ToString()), DateTime.Parse(this.txtStartDate.Text + " 00:00:00"), DateTime.Parse(this.txtEndDate.Text + " 23:59:59"), Convert.ToInt32(ddlChannel.SelectedValue), Convert.ToInt32(ddlArea.SelectedValue), Convert.ToInt32(ddlCustomer.SelectedValue), int.Parse(this.Session["UserId"].ToString()), Convert.ToInt32(ddlCity.SelectedValue));
        DataTable dt = mDocumentPrntControl.SelectReportTitle(int.Parse(drpDistributor.SelectedValue.ToString()));
        SAMSBusinessLayer.Reports.CrpRegionSKU CrpReport = new SAMSBusinessLayer.Reports.CrpRegionSKU();
    ...
    protected void cblCategory_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if(selectCount == cblCategory.Items.Count)
        {
