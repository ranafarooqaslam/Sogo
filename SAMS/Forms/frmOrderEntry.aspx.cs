...
public partial class Forms_frmOrderEntry : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            foreach (ListItem li in RblPayMode.Items)
            {
                li.Selected = false;
                li.Enabled = false;
            }
            string[] UserInvoiceType = Session["UserInvoiceType"].ToString().Split(',');
            int count = 0;
            foreach(string s in UserInvoiceType)
            {
                foreach (ListItem li in RblPayMode.Items)
                {
                    if(li.Value == s)
    ...
    protected void drpDocumentNo_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (long.Parse(drpDocumentNo.SelectedValue.ToString()) != Constants.LongNullValue)
        {
            ...
            {
                drpAssignedDistributor.Enabled = false;
                this.FindCustomer();
                ...
                try
                {
                    drpAssignedDistributor.SelectedValue = foundRows[0]["ASSIGNED_DISTRIBUTOR_ID"].ToString();
                }
                catch (Exception ex)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "msg", "alert('Stock Used By Location not found');", true);
                    return;
                }
    ...
    private void LoadAssingned()
    {
        ...
        drpAssignedDistributor.Items.Clear();
        if (this.Session["DistributorId"] != null)
        {
            DataTable dt = mUserController.SelectDisToDisAssignment(int.Parse(this.Session["DistributorId"].ToString()), Constants.IntNullValue, 8, int.Parse(this.Session["CompanyId"].ToString()), Convert.ToDateTime(Session["CurrentWorkDate"]));
            clsWebFormUtil.FillDropDownList(drpAssignedDistributor, dt, 0, 2);
        }
    }
    ...
    private void EnableDisableController(bool CValue)
    {
        ...
        if (CValue == true)
        {
            ...
            {               
                ...
                drpAssignedDistributor.Enabled = true;
            }
        }
        else
        {            
    ...
    protected void btnSaveOrder_Click(object sender, EventArgs e)
    {
        ...
        if(ModoeCount == 0)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "msg", "alert('Invoice Type not selected !');", true);
            return;
        }
        GrdPurchase.EditIndex = -1;
        if (drpAssignedDistributor.Items.Count > 0)
        { }
        else
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "msg", "alert('Assigned Location Not found !');", true);
            return;
        }
        if (FindCustomer())
        {
            ...
            else
            {
                ...
                else if (btnSaveOrder.Text == "Save Invoice")
                {
                    ...
                    PurchaseSKU, dtFreeSKU, int.Parse(this.Session["UserId"].ToString()), decimal.Parse(DC.chkNull_0(txtCashReceived.Text)), DateTime.Parse(this.Session["OrderDate"].ToString()), decimal.Parse(DC.chkNull_0(numTxtTotalTST.Text)), decimal.Parse(DC.chkNull_0(numtxtUnClaimabledist.Text)),txtRemarks.Text,Convert.ToInt32(drpAssignedDistributor.SelectedValue));
                    if (IsValidInsert)
                    {
                        this.LoadCustomerData();
                        this.LoadPendingOrder();
    ...
    protected void drpAssignedDistributor_SelectedIndexChanged(object sender, EventArgs e)
    {
        GrdPurchase.EditIndex = -1;
        LoadSKUDetail();
    }
    ...
    protected void GrdPurchase_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName.ToLower() == "edt")
        {
            ...
            int stock = 0;
            stock += Convert.ToInt32(GrdPurchase.Rows[index].Cells[10].Text) * Convert.ToInt32(GrdPurchase.Rows[index].Cells[13].Text);
            stock += Convert.ToInt32(GrdPurchase.Rows[index].Cells[11].Text);

            string[] strstock = GrdPurchase.Rows[index].Cells[12].Text.Split('-');
            stock += Convert.ToInt32(strstock[0]) * Convert.ToInt32(GrdPurchase.Rows[index].Cells[13].Text);
            stock += Convert.ToInt32(strstock[1]);
            ...
            txtExtDiscount.Text = GrdPurchase.Rows[index].Cells[7].Text;
            txtUnitRate.Text = GrdPurchase.Rows[index].Cells[5].Text;
            txtStock.Text = (stock / Convert.ToInt32(GrdPurchase.Rows[index].Cells[13].Text)).ToString() + "-" + (stock % Convert.ToInt32(GrdPurchase.Rows[index].Cells[13].Text)).ToString();
            btnSave.Text = "Update SKU";
        }
    }
}